...
@interface DMVideoNews ()

@end

@implementation DMVideoNews
...
- (void)viewDidLoad
{
    ...
    //视频播放设置
    self.imageView =[[UIImageView alloc] initWithFrame:CGRectMake(10, self.ymVideoZeroHight, 300, 250)];
    [self.imageView setBackgroundColor:[UIColor blueColor]];
    NSURL *tempURL = [NSURL URLWithString:self.ymVideoModel.ymThumb];
    [self.imageView setImage: [DMJSONParse gmRequestImageNews:tempURL]];
//    [self.imageView setAlpha:0.6];
    ...
    [self.ymScrollV addSubview:self.imageView];
    
    
    //真机调试
    
    //解码器实例化
//    self.video = [[FrameExtractor alloc] initWithVideo:@"http://192.168.0.43:8080/Video/BaoZouNvRen.mp4"] ;
//    self.video = [[FrameExtractor alloc] init];
...
//    lastFrameTime = -1;
//    [self.video setupCgimageSession];
//    // 查看视频源属性
...
- (void) handleTap2:(UITapGestureRecognizer*) recognizer {
    ...
    self.imageView.frame = CGRectMake(10, self.ymVideoZeroHight, 300, 250);
...
#pragma markMPMediaPickerControllerDelegate
...
-(void)gmDisplayBasicModel{
    ...
    [labTitle setText:[NSString stringWithFormat:@"    %@",self.ymVideoModel.ymTitle]];
    ...
    NSString *strTimeOne = [self.ymVideoModel.ymPublished substringToIndex:10];
    NSString *strTimeTwo = [self.ymVideoModel.ymPublished substringFromIndex:[self.ymVideoModel.ymPublished length]-8];
    NSString *strSource = [NSString stringWithFormat:@"来源：%@ %@ %@",self.ymVideoModel.ymSource,strTimeOne,strTimeTwo];
    ...
    //新闻图片.视频原点高度位置
    self.ymVideoZeroHight = labSource.frame.origin.y+labSource.bounds.size.height+10;
    UIImageView *basicImg = [[UIImageView alloc] initWithFrame:CGRectMake(10, self.ymVideoZeroHight, 300, 250)];
    ...
    [descriptionlab setText:self.ymVideoModel.ymDescription];
...
-(void)gmSetControlSubView{
    //视图头部 设置
    if(!self.ymVideoTitleUpV){
        self.ymVideoTitleUpV = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 320, 40)];
        [self.ymVideoTitleUpV setAlpha:0.8];
        [self.view addSubview:self.ymVideoTitleUpV];
    }
    ...
    [self.ymVideoTitleUpV addSubview:tempBackV];
...
-(void)gmButtonDownRightAction{
    if([self.ymTxtDownUpBut.text length] != 0){
        ...
        //发表评论,附上新闻ID，和跟帖内容
        NSString *strAO = [DMAppearObserve gmFromAppearObserveToJSONContentid:self.ymVideoModel.ymContentid andContent:self.ymTxtDownUpBut.text];
        
        NSString *strJudge = [strAO substringWithRange:NSMakeRange(12, 5)];
        
        //若评论成功，操作
        if([strJudge intValue] == 10000){
            UIAlertView *alertSuccess = [[UIAlertView alloc] initWithTitle:@"评论" message:@"发表评论成功" delegate:self cancelButtonTitle:@"返回" otherButtonTitles:nil, nil];
            [alertSuccess show];
        }else{
            //评论失败
            UIAlertView *alertSuccess = [[UIAlertView alloc] initWithTitle:@"评论" message:@"发表评论失败" delegate:self cancelButtonTitle:@"返回" otherButtonTitles:nil, nil];
            [alertSuccess show];
        }
...
-(void)gmSubViewButAction:(id)sender{
    ...
    if(tempBut.tag == 600){
        ...
        NSMutableArray *tempVideoCollection = [DMCollectionSave gmSharedNewsCollection];
        BOOL result = YES;
        for(int i = 0; i < [tempVideoCollection count]; i ++){
            DMNewsDataModel *tempModel = (DMNewsDataModel *)[tempVideoCollection objectAtIndex:i];
            if([tempModel.ymContentid intValue] == [self.ymVideoModel.ymContentid intValue]){
                
                result = NO;
            }
        }
        if(result){
            [DMCollectionSave gmSharedNews:self.ymVideoModel];
        }
...
-(void)gmVideoAction{
    ...
    [basicBut setTitle:[NSString stringWithFormat:@"%d跟帖",[self.ymVideoModel.ymComments intValue]] forState:UIControlStateNormal];
    ...
    //跟帖关联事件
    [basicBut addTarget:self action:@selector(gmCommentsVideo) forControlEvents:UIControlEventTouchUpInside];
    [self.ymVideoTitleUpV addSubview:basicBut];
    ...
    [self.ymVideoTitleUpV addSubview:rightBut];
...
//跟帖关联事件
-(void)gmCommentsVideo{
    self.ymCommentsVideo = nil;
    if(!self.ymCommentsVideo){
        self.ymCommentsVideo = [[DMComments alloc] init];
        [self.ymCommentsVideo.view setFrame:CGRectMake(320, 0, 320, 460)];
        
         self.ymCommentsVideo.ymContentid = [self.ymVideoModel.ymContentid intValue];
        //响应通知
        NSNotification *notification = [[NSNotification alloc] initWithName:@"gmCommentList" object:nil userInfo:nil];
        [[NSNotificationCenter defaultCenter] postNotification:notification];
          [self.view addSubview:self.ymCommentsVideo.view];
    }
    [UIView animateWithDuration:0.2 animations:^{
        [self.ymCommentsVideo.view setFrame:CGRectMake(0, 0, 320, 460)];
    } completion:^(BOOL finished){}];
